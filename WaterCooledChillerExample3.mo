...
partial model WaterCooledChillerExample3 "Primary only chiller plant system with water-side economizer"
  ...
  parameter Modelica.SIunits.Power QRoo_flow = 500000 "Heat generation of the computer room";
  ...
  Buildings.Fluid.Sensors.TemperatureTwoPort TAirSup(redeclare package Medium = MediumA, m_flow_nominal = mAir_flow_nominal) "Supply air temperature to data center" annotation(
    Placement(visible = true, transformation(origin = {264, -225}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
...
equation
  ...
  connect(TAirSup.port_a, fan.port_b) annotation(
    Line(points = {{274, -225}, {328, -225}}, color = {0, 127, 255}, thickness = 0.5));
  ...
  connect(TRooAir.port, vol.heatPort) annotation(
    Line(points = {{256, -274}, {226, -274}, {226, -206}}, color = {191, 0, 0}));
  connect(QSou.port, vol.heatPort) annotation(
    Line(points = {{198, -274}, {226, -274}, {226, -206}}, color = {191, 0, 0}));
  connect(ramp.y, QSou.Q_flow) annotation(
    Line(points = {{158, -274}, {178, -274}}, color = {0, 0, 127}));
  connect(theCon.port_a, TOut.port) annotation(
    Line(points = {{176, -236}, {148, -236}}, color = {191, 0, 0}));
  connect(theCon.port_b, vol.heatPort) annotation(
    Line(points = {{196, -236}, {226, -236}, {226, -206}}, color = {191, 0, 0}));
  connect(TOut.T, weaBus.TDryBul) annotation(
    Line(points = {{126, -236}, {-322, -236}, {-322, -88}}, color = {0, 0, 127}));
  connect(TAirSup.port_b, vol.ports[1]) annotation(
    Line(points = {{254, -225}, {236, -225}, {236, -196}}, color = {0, 127, 255}));
  connect(cooCoi.port_a2, vol.ports[2]) annotation(
    Line(points = {{280, -176}, {236, -176}, {236, -196}}, color = {0, 127, 255}));
  annotation(
...
end WaterCooledChillerExample3;
